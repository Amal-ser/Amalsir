...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'Tinsonsir' || config.AFPLK == 'afamk' || config.AFPLK == 'vava' || config.AFPLK == 'Tinsonsir') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'Tinsonsir' || config.AFPLK !== 'afamk' || config.AFPLK !== 'vava' || config.AFPLK !== 'Tinsonsir') {
         console.log(
        ...
        console.log(
            chalk.green.bold('Tinsonsir 𝚠𝚘𝚛𝚔𝚒𝚗𝚐 ' + config.WORKTYPE + ' 𝚗𝚘𝚠 🍃'));
          // thanks to afnanplk
    ...
    if (config.SUPPORT == '917994088426-918301813739') {     
...
*/
                        ...
                        try {
                            ...
                            if (config.NOLOG === 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ Tinsonsir ~______~*' +
